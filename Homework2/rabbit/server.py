...
class Server:
    def __init__(self):
        ...
        self.ch.queue_declare(queue='rpc_queue', exclusive=True)
        self.ch.exchange_declare(exchange='direct_notify', exchange_type='direct')
        self.ch.exchange_declare(exchange='direct_rpc', exchange_type='direct')

        self.ch.queue_bind(exchange='direct_rpc', queue='rpc_queue', routing_key='rpc_queue')
        self.ch.basic_consume(self.on_request, queue='rpc_queue')
    ...
    def stop(self):
        if len(self.clients) != 0:
            self.notify_clients('Stopping', 'Stopping', 'all_clients')
    ...
    def notify_clients(self, header, msg, routing='all_clients'):
        ...
        self.ch.basic_publish(exchange='direct_notify', routing_key=routing, body=msg, )

    def notify_named_clients(self, header, msg, client_names):
        msg = header + ':' + msg
        for c in client_names:
            print 'NOTIFY - key [%s] - msg [%s]' % (c, msg)
            self.ch.basic_publish(exchange='direct_notify', routing_key=c, body=msg, )
